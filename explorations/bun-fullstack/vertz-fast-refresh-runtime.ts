...
// Import from the BUILT dist of @vertz/ui to share the SAME module instance
// as the app code. Using source paths would create separate `currentScope` /
// disposal stack variables, breaking context capture and cleanup during HMR.
import {
  _tryOnCleanup,
  getContextScope,
  popScope,
  ...
  runCleanups,
  setContextScope,
} from '../../packages/ui/dist/internals.js';

/** Disposal cleanup function. */
type DisposeFn = () => void;

/** A snapshot of context values at a point in time. */
type ContextScope = Map<{ Provider: unknown }, unknown>;
...
export function __$refreshReg(
  ...
  if (existing) {
    ...
    // Always mark as dirty: Bun's targeted HMR only re-evaluates the changed
    // file, so if __$refreshReg is called again, the file DID change.
    // NOTE: We cannot use factory.toString() comparison because the factory
    // is the wrapper function (same boilerplate every time). The actual
    // component code is in a closure variable that toString() doesn't capture.
...
export function __$refreshPerform(moduleId: string): void {
  ...
  // On first load, modules are not dirty — __$refreshReg creates fresh records.
  // On HMR re-evaluation, only the changed module is dirty.
  ...
  for (const [name, record] of mod) {
    ...
    for (const instance of instances) {
      ...
      //    This way, if the factory throws, we haven't destroyed the old component.
      ...
      try {
        ...
        // Factory failed — keep old instance intact, don't destroy it.
        popScope();
...
// can access them WITHOUT importing this module. This prevents Bun's HMR
// from propagating updates through @vertz/ui/dist (which this module
// imports but component modules should NOT).
//
// The plugin injects inline code that reads from globalThis.__VERTZ_FR__
// instead of adding import statements.
...
(globalThis as Record<symbol, unknown>)[FR_KEY] = {
  ...
  __$refreshTrack,
  __$refreshPerform,
  // Disposal and context scope functions — needed by the injected wrapper code.
  // These come from @vertz/ui/internals (same module instance as the app uses).
  pushScope,
  popScope,
  _tryOnCleanup,
  runCleanups,
