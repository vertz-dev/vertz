import type { HydrationOptions, RawHtml, VNode } from './types';
 ...
 */
export function wrapWithHydrationMarkers(node: VNode, options: HydrationOptions): VNode {
  ...
  const newChildren: (VNode | string | RawHtml)[] = [...node.children];

  if (options.props !== undefined) {
    const propsScript: VNode = {
      tag: 'script',
