...
Models with a `tenant` declaration are automatically filtered by the current tenant context at query time. The framework also follows relation chains for indirect scoping — if `tasks` belongs to `projects` which belongs to `organizations`, and only `projects` declares the tenant, tasks are scoped transitively.
