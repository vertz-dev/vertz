...
Each model becomes a property on `db` with typed query methods.
...
|----------|--------|-------|
| PostgreSQL | `vertz/db` | Default dialect, full feature support |
| SQLite | `vertz/db/sqlite` | Development and embedded use |
| Cloudflare D1 | `vertz/db/d1` | Edge deployment |
