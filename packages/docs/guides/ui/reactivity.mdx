...
return (
  <button
    class={isActive ? 'btn-primary' : 'btn-ghost'}
    disabled={isDisabled}
...
Each attribute updates independently. Changing `isActive` updates `class`, `aria-pressed`, and `style` — but doesn't touch `disabled`.
