...
Compound components are groups of related components designed to work together — like `Card` with `CardHeader`, `CardContent`, and `CardFooter`, or `Dialog` with `Dialog.Trigger` and `Dialog.Content`.
...
```tsx
// Wrong — inspecting children to rearrange them
export function Card() {
  const kids = children();
  const header = kids.filter(isCardHeader);
  const body = kids.filter(isCardBody);
  const footer = kids.filter(isCardFooter);
