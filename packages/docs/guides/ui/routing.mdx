...
Vertz routing is fully typed — route patterns, parameters, and navigation are all checked at compile time. If you add a route for `/tasks/:id`, then `navigate('/tasks/:id', { params: { id: 'abc' } })` compiles but `navigate('/bogus')` doesn't.
...
navigate('/');              // OK
navigate('/tasks/:id', { params: { id: 'abc' } }); // OK
navigate('/bogus');         // Type error
navigate('/tasks/:id');     // Type error — params required
```
