...
These components use an imperative factory API — they return DOM elements rather than JSX components. Available via `components.primitives.*` after calling `configureTheme()`.

| Component | Description |
|-----------|-------------|
| Checkbox | Toggle control for boolean values |
| Switch | Toggle between on and off states |
| RadioGroup | Select one option from a set |
| Slider | Range input with track and thumb |
| Toggle | Toggle button with pressed state |
| Progress | Task completion percentage indicator |
| Toast | Temporary notification popup |

## Additional primitives

These components are also available via `components.primitives.*` but are less commonly used:
...
For custom styling, you can import headless primitives directly from `@vertz/ui-primitives` and apply your own classes. See [Styling](/guides/ui/styling) for details on `css()` and `variants()`.
