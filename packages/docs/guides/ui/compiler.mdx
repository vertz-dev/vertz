...
```tsx
// You write:
<div class={styles.card}>{title}</div>

// Compiler outputs:
const el = document.createElement('div');
el.className = styles.card;
effect(() => { el.textContent = title.value; });
```
...
// Compiler outputs:
TaskCard({
  get task() { return tasks.value[0]; },
  get disabled() { return isLoading.value; },
});
```
...
function TaskCard({ title, completed }: TaskCardProps) {
  return <div class={completed ? 'done' : ''}>{title}</div>;
}

// Compiler outputs:
function TaskCard(__props: TaskCardProps) {
  return <div class={__props.completed ? 'done' : ''}>{__props.title}</div>;
}
```
...
// becomes:
effect(() => { el.textContent = tasks.data.value?.items.length; });
```
