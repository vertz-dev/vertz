...
These are signals — they auto-unwrap in JSX:
...
async function handleDelete(id: string) {
  ...
  tasks.refetch(); // Clear cache and re-fetch
}
```
...
// Revalidate all active todo list queries
...
`invalidate()` triggers a background refetch (SWR pattern) — existing data stays visible while the fresh data loads. It accepts the same query descriptors used with `query()`.
