...
<Note>Before hooks are for data enrichment, not data transformation. The return type must match the input type — you can set or modify values within the schema's shape, but you can't add new fields or change field types. The schema defines the shape, the hook fills in values.</Note>
...
<Note>Every entity's model must be registered in `createDb({ models })`. This is validated at two levels: the **compiler** catches missing models at build time (flagging `ENTITY_MODEL_NOT_REGISTERED` errors), and `createServer()` validates again at startup with a clear error listing all missing models. You'll never hit a cryptic runtime failure on first request.</Note>
