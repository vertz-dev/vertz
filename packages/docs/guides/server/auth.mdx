...
<Note>`rules.where()` inside `rules.any()` is still evaluated in memory, not at the database level. Database extraction only works with AND logic (`rules.all()`), because OR logic could bypass enforcement.</Note>
