...
Vertz's code generator reads your `defineAccess()` configuration at build time and produces two artifacts:

1. **`access.d.ts`** — TypeScript declarations that make `ctx.can('typo')` a compile error
2. **`rls-policies.sql`** — PostgreSQL Row-Level Security policies from `rules.where()` conditions

Both are generated into `.vertz/generated/` alongside your other codegen output.
...
If your entitlements use `rules.where()` conditions, the code generator can produce PostgreSQL Row-Level Security (RLS) policies automatically.
...
### Using the generated policies

The generated `rls-policies.sql` is a starting point. Apply it to your database as part of your migration workflow:
...
<Steps>
  ...
  <Step title="Set session variables">
    Your application must set `app.user_id` and `app.tenant_id` on each database connection before running queries:
