...
RLS policies reference `app.user_id` and `app.tenant_id` via `current_setting()`. Use `withSessionVars()` from `@vertz/db` to set these within a transaction:
...
`withSessionVars()` validates UUIDs, wraps the callback in a transaction, and uses `SET LOCAL` so variables are scoped to the transaction and automatically cleared on commit or rollback.
