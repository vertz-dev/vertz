...
<Note>`safeParse` is the recommended approach — it aligns with Vertz's errors-as-values philosophy. Use `parse` only in contexts where throwing is acceptable (test setup, scripts).</Note>
