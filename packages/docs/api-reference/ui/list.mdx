...
## Migration from ListTransition

`ListTransition` is deprecated. Migrate to `<List animate>`:
...
Key differences:
- `<List>` uses `.map()` children instead of `each`/`keyFn`/`children` props
- `.map()` preserves VertzQL auto field selection
- `<List>` adds FLIP reorder animations (not just enter/exit)
- `<List>` supports drag-and-sort via `sortable` prop
