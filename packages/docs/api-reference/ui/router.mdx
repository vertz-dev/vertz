...
| `navigate(to, options?)` | `<TPath extends keyof T & string>(to: TPath, options?) => Promise<void>` | Navigate to a route pattern with typed params |
...
function TaskCard({ taskId }: { taskId: string }) {
  ...
  const handleClick = () => {
    router.navigate('/tasks/:id', { params: { id: taskId } });
  };
