...
export function buildSchemaRegistry(ir: AppIR): SchemaRegistryManifest {
  const schemas: SchemaRegistryEntry[] = ir.schemas
    .filter((s) => s.isNamed)
    .map((s) => ({
      name: s.name,
      ...(s.id && { id: s.id }),
...
function indentJson(value: Record<string, unknown>, indent: string): string {
...
export function renderSchemaRegistryFile(
  manifest: SchemaRegistryManifest,
  outputDir: string,
): string {
  const lines: string[] = ['// Auto-generated by @vertz/compiler — do not edit', ''];
