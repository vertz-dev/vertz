...
export class ComputedTransformer {
	transform(
		...
		const computeds = new Set(
			variables.filter((v) => v.kind === "computed").map((v) => v.name),
		);
			...
			for (const decl of declList.getDeclarations()) {
				...
				if (nameNode.isKind(SyntaxKind.ObjectBindingPattern)) {
					...
					if (syntheticName) {
						...
						const signalProps =
							synthetic?.signalProperties ?? new Set<string>();
						...
						for (const el of elements) {
							...
							const propName =
								el.getPropertyNameNode()?.getText() ?? bindingName;
							if (computeds.has(bindingName) && signalProps.has(propName)) {
								...
								lines.push(
									`const ${bindingName} = ${syntheticName}.${propName}`,
								);
							}
						}
						source.overwrite(
					...
					const computedElements = elements.filter((el) =>
						computeds.has(el.getName()),
					);
...
function transformComputedReads(
	source: MagicString,
	bodyNode: Node,
	computeds: Set<string>,
): void {
