...
export class SignalTransformer {
	transform(
		...
		const signals = new Set(
			variables.filter((v) => v.kind === "signal").map((v) => v.name),
		);
...
function transformSignalApiProperties(
	...
	bodyNode.forEachDescendant((node) => {
		...
		if (parent?.isKind(SyntaxKind.PropertyAccessExpression)) {
			...
			if (
				parentProp.getExpression() === outerExpr &&
				parentProp.getName() === "value"
			) {
	...
	bodyNode.forEachDescendant((node) => {
		...
		if (fieldChainRanges.some((r) => nodeStart >= r.start && nodeStart < r.end))
			return;
