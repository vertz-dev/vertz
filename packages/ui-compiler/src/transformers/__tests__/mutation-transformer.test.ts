...
describe('MutationTransformer', () => {
  ...
  it('handles self-referential mutations: items.push(items.length)', () => {
    const code = `function App() {\n  let items = [];\n  items.push(items.length);\n  return <div>{items}</div>;\n}`;
    const result = transform(code, [{ name: 'items', kind: 'signal', start: 0, end: 0 }]);
    expect(result).toContain('items.peek().push(items.peek().length)');
  });

  it('works inside expression-body arrow functions', () => {
    const code = `function App() {\n  let items = [];\n  const add = () => items.push("x");\n  return <div>{items}</div>;\n}`;
    const result = transform(code, [{ name: 'items', kind: 'signal', start: 0, end: 0 }]);
    // Comma expression is valid in arrow expression bodies
    expect(result).toContain('() => (items.peek().push("x"), items.notify())');
