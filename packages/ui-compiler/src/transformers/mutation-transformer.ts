...
export class MutationTransformer {
  ...
  private _transformMethodCall(
    ...
    // Replace first occurrence of `varName.` with `varName.peek().`
    const peekText = originalText.replace(`${variableName}.`, `${variableName}.peek().`);
  ...
  private _transformObjectAssign(
    ...
    const { variableName } = mutation;
    // Replace `Object.assign(varName,` with `Object.assign(varName.peek(),`
