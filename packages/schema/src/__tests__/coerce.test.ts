...
import { coerceFormDataToSchema, coerceLeaf, isVertzSchema, s } from '..';
...
describe('Feature: coerceLeaf — Boolean inner schema', () => {
  ...
  describe('Given the value is null', () => {
    it('then returns false', () => {
      expect(coerceLeaf(null, s.boolean())).toBe(false);
    });
  });
  describe('Given the value is a non-string truthy value', () => {
...
describe('Feature: coerceLeaf — Number inner schema', () => {
  ...
  describe('Given a non-string non-number value', () => {
...
describe('Feature: coerceLeaf — BigInt inner schema', () => {
  ...
  describe('Given the value is a number', () => {
    it('then returns BigInt(n)', () => {
      expect(coerceLeaf(7, s.bigint())).toBe(7n);
    });
  });
  describe('Given the value is already a bigint', () => {
    it('then returns the bigint unchanged', () => {
      expect(coerceLeaf(7n, s.bigint())).toBe(7n);
    });
  });
  describe('Given a non-string non-number non-bigint value', () => {
    it('then passes the value through', () => {
      const obj = { x: 1 };
...
describe('Feature: coerceFormDataToSchema — FormData input', () => {
...
describe('Feature: coerceFormDataToSchema — URLSearchParams input', () => {
  describe('Given a urlencoded body (x-www-form-urlencoded)', () => {
    it('then coerces boolean and number fields just like FormData', () => {
      const schema = s.object({
        name: s.string(),
        active: s.boolean(),
        priority: s.number(),
      });
      const params = new URLSearchParams('name=Buy+milk&active=on&priority=3');
      expect(coerceFormDataToSchema(params, schema)).toEqual({
        name: 'Buy milk',
        active: true,
        priority: 3,
      });
  ...
  describe('Given a urlencoded body with unchecked boolean (key absent)', () => {
    it('then still sets the boolean to false', () => {
      const schema = s.object({ active: s.boolean() });
      const params = new URLSearchParams('');
      expect(coerceFormDataToSchema(params, schema)).toEqual({ active: false });
    });
  });

  describe('Given a urlencoded body with array of primitives', () => {
    it('then collects repeated keys via getAll', () => {
      const schema = s.object({ tags: s.array(s.string()) });
      const params = new URLSearchParams('tags=a&tags=b&tags=c');
      expect(coerceFormDataToSchema(params, schema)).toEqual({ tags: ['a', 'b', 'c'] });
    });
  });
});
...
describe('Feature: coerceFormDataToSchema — non-Vertz adapter fallback', () => {
  describe('Given a custom adapter without _schemaType', () => {
    it('then falls back to nested form-data parsing', () => {
      const adapter = { parse: (v: unknown) => v };
      const fd = new FormData();
      fd.append('a', '1');
      fd.append('b', '2');
  ...
  describe('Given a FormData with dotted keys and no Vertz schema', () => {
    it('then still produces a nested object by dot-path', () => {
      const fd = new FormData();
      fd.append('address.street', 'Main St');
      fd.append('address.zip', '12345');
      expect(coerceFormDataToSchema(fd, null)).toEqual({
...
describe('Feature: coerceFormDataToSchema — prototype pollution safety', () => {
  describe('Given a FormData with __proto__ key', () => {
    it('then refuses to set it when falling back to nested parsing', () => {
      const fd = new FormData();
      fd.append('__proto__.polluted', 'true');
      const result = coerceFormDataToSchema(fd, null);
