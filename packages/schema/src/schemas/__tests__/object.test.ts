...
describe('ObjectSchema', () => {
  ...
  it('strips unknown keys by default', () => {
    const schema = new ObjectSchema({ name: new StringSchema() });
    const result = schema.parse({ name: 'Alice', extra: 'ignored', another: 42 }).data;
  ...
  it('.passthrough() preserves unknown keys in output', () => {
    ...
    const result = schema.parse({ name: 'Alice', extra: 'kept', another: 42 }).data;
  ...
  it('.catchall(schema) validates unknown keys against the catchall schema', () => {
    ...
    expect(schema.parse({ name: 'Alice', score: 100 }).data).toEqual({ name: 'Alice', score: 100 });
  ...
  it('.extend(shape) adds new properties', () => {
    ...
    expect(extended.parse({ name: 'Alice', age: 30 }).data).toEqual({ name: 'Alice', age: 30 });
  ...
  it('.merge(otherObject) combines two object schemas, later shape wins on conflict', () => {
    ...
    expect(merged.parse({ name: 'Alice', age: '30', email: 'a@b.com' }).data).toEqual({
      name: 'Alice',
      age: '30',
      email: 'a@b.com',
    });
  ...
  it('.pick(keys) keeps only specified keys', () => {
    ...
    expect(picked.parse({ name: 'Alice', email: 'a@b.com' }).data).toEqual({
      name: 'Alice',
      email: 'a@b.com',
    });
  ...
  it('.omit(keys) removes specified keys', () => {
    ...
    expect(omitted.parse({ name: 'Alice', email: 'a@b.com' }).data).toEqual({
      name: 'Alice',
      email: 'a@b.com',
    });
  ...
  it('.required() unwraps OptionalSchema wrappers', () => {
    ...
    expect(required.parse({ name: 'Alice', age: 30 }).data).toEqual({ name: 'Alice', age: 30 });
  });
