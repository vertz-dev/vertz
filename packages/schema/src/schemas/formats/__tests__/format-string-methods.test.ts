...
describe('format schemas inherit string methods', () => {
  describe('EmailSchema', () => {
    ...
    it('supports .min()', () => {
      ...
      expect(schema.parse('user@longdomain.com').data).toBe('user@longdomain.com');
    });

    it('supports .max()', () => {
      const schema = s.email().max(10);
      expect(schema.safeParse('verylongemail@domain.com').ok).toBe(false);
      expect(schema.parse('a@b.co').data).toBe('a@b.co');
    });

    it('supports .length()', () => {
      const schema = s.email().length(14);
      expect(schema.safeParse('a@b.co').ok).toBe(false);
      expect(schema.parse('user@domain.co').data).toBe('user@domain.co');
    });

    it('supports .regex()', () => {
      const schema = s.email().regex(/^[a-z]+@/);
      expect(schema.safeParse('ABC@domain.com').ok).toBe(false);
      expect(schema.parse('abc@domain.com').data).toBe('abc@domain.com');
    });

    it('supports .startsWith()', () => {
      const schema = s.email().startsWith('admin');
      expect(schema.safeParse('user@domain.com').ok).toBe(false);
      expect(schema.parse('admin@domain.com').data).toBe('admin@domain.com');
    });

    it('supports .endsWith()', () => {
      const schema = s.email().endsWith('.com');
      expect(schema.safeParse('user@domain.org').ok).toBe(false);
      expect(schema.parse('user@domain.com').data).toBe('user@domain.com');
    });

    it('still rejects invalid emails after chaining', () => {
      const schema = s.email().trim().toLowerCase();
      expect(schema.safeParse('  not-an-email  ').ok).toBe(false);
    });
