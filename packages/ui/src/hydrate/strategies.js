...
export function interactionStrategy(el, hydrateFn) {
...
export function idleStrategy(el, hydrateFn) {
  void el;
 ...
 */
export function mediaStrategy(query) {
  return (el, hydrateFn) => {
    void el;
...
export function visibleStrategy(el, hydrateFn) {
  if (typeof IntersectionObserver === 'undefined') {
    // Fallback to eager when IntersectionObserver is not available
    hydrateFn();
    return;
