...
export class DbOAuthAccountStore implements OAuthAccountStore {
  ...
  async linkAccount(
    ...
    // Use ON CONFLICT DO NOTHING for idempotent link (UNIQUE(provider, provider_id) handles dedup)
    const result = await this.db.query(
      sql`INSERT INTO auth_oauth_accounts (id, user_id, provider, provider_id, email, created_at)
