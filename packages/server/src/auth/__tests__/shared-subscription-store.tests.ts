...
export function subscriptionStoreTests(
  ...
  describe(`SubscriptionStore: ${name}`, () => {
    ...
    it('overwrites existing subscription for same (resourceType, resourceId)', async () => {
      await store.assign('tenant', 'tenant-1', 'free');
    ...
    it('attachAddOn and getAddOns', async () => {
      ...
      await store.attachAddOn('tenant', 'tenant-1', 'addon-a');
      await store.attachAddOn('tenant', 'tenant-1', 'addon-b');

      const addOns = await store.getAddOns('tenant', 'tenant-1');
    ...
    it('detachAddOn removes an add-on', async () => {
      ...
      await store.attachAddOn('tenant', 'tenant-1', 'addon-a');
      await store.attachAddOn('tenant', 'tenant-1', 'addon-b');
      await store.detachAddOn('tenant', 'tenant-1', 'addon-a');

      const addOns = await store.getAddOns('tenant', 'tenant-1');
      expect(addOns).toEqual(['addon-b']);
    });

    it('getAddOns returns empty for unknown resource', async () => {
      if (!store.getAddOns) return;

      const addOns = await store.getAddOns('tenant', 'tenant-unknown');
