...
// ---------------------------------------------------------------------------
// SQL Generation from Schema (same as SQLite - could be shared)
// ---------------------------------------------------------------------------
...
function generateCreateTableSql<T extends ColumnRecord>(schema: TableDef<T>): string {
  ...
  for (const [colName, colBuilder] of Object.entries(schema._columns)) {
    ...
    let colDef = `${colName} ${getD1Type(meta)}`;
...
/**
 * Get D1/SQLite type string from column metadata (D1 uses SQLite under the hood).
 */
function getD1Type(meta: ColumnMetadata): string {
  switch (meta.sqlType) {
    case 'serial':
      return 'INTEGER';
    case 'varchar':
 ...
 */
export class D1Adapter<T extends ColumnRecord> implements EntityDbAdapter {
    ...
    try {
      ...
      for (const [colName, colBuilder] of Object.entries(this.schema._columns)) {
        ...
        if (data[colName] !== undefined || meta.hasDefault) {
          ...
          // Convert boolean to integer for D1 (SQLite backend)
          if (meta.sqlType === 'boolean') {
            value = value ? 1 : 0;
          }
