...
export interface DerivedSchemas {
  /** Excludes columns with defaults (including auto-generated PKs). Composite PK columns without defaults are included. For POST bodies. */
  ...
  /**
   * Strict API create input schema. Excludes auto-generated PKs (primary + hasDefault),
   * readOnly, hidden columns. Composite PK columns (primary without hasDefault) are
   * INCLUDED as required. Columns with defaults are optional. Rejects unknown keys.
   */
...
export function tableToSchemas<TColumns extends ColumnRecord>(
  ...
  for (const [columnName, columnBuilder] of Object.entries(table._columns)) {
    ...
    // createBody: exclude columns with defaults (auto-generated PKs have hasDefault=true,
    // composite PK columns have hasDefault=false and are included as required)
    if (!meta.hasDefault) {
    ...
    // API create: exclude auto-generated PKs (primary + hasDefault), readOnly, hidden
    // Composite PK columns (primary without hasDefault) are INCLUDED as required.
