...
describe('Aggregation queries (DB-012)', () => {
  ...
  describe('aggregate', () => {
    it('computes _avg correctly', async () => {
      ...
      expect((result.data._avg as Record<string, unknown>).price).toBe(200);
    });
    ...
    it('computes _sum correctly', async () => {
      ...
      // 10 + 5 + 3 + 8 + 12 = 38
      expect((result.data._sum as Record<string, unknown>).quantity).toBe(38);
    });
    ...
    it('computes _min and _max correctly', async () => {
      ...
      expect((result.data._min as Record<string, unknown>).price).toBe(100);
      expect((result.data._max as Record<string, unknown>).price).toBe(300);
    });
    ...
    it('respects where filter on aggregate', async () => {
      ...
      expect(result.data._count).toBe(3);
      // (100 + 150 + 200) / 3 = 150
      expect((result.data._avg as Record<string, unknown>).price).toBe(150);
    });
  ...
  describe('groupBy', () => {
    it('groups by specified columns', async () => {
      ...
      const gadgets = result.data.find((r) => r.category === 'gadgets');
    ...
    it('respects orderBy on aggregation results', async () => {
      ...
      expect(result.data[0]?._count).toBe(3);
      expect(result.data[1]?.category).toBe('gadgets');
      expect(result.data[1]?._count).toBe(2);
    });
  ...
  describe('groupBy orderBy safety (B1)', () => {
    it('rejects underscore-prefixed orderBy columns that are not valid aggregation aliases', async () => {
      ...
      expect(result.error.message).toMatch(/Invalid orderBy/);
    });
    ...
    it('rejects orderBy with an invalid direction value', async () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.error.message).toMatch(/Invalid orderBy direction/);
    });
