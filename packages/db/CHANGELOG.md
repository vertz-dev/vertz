...
- [#2891](https://github.com/vertz-dev/vertz/pull/2891) [`cf25bbb`](https://github.com/vertz-dev/vertz/commit/cf25bbb270186e25ca34a11f29d361e7113412bc) Thanks [@viniciusdacal](https://github.com/viniciusdacal)! - feat(db): run `d.jsonb<T>({ validator })` on write paths across Postgres and SQLite/D1
...
- [#2870](https://github.com/vertz-dev/vertz/pull/2870) [`d0a9a2f`](https://github.com/vertz-dev/vertz/commit/d0a9a2fd27c2c69b5cddd4e5eca822915336ba53) Thanks [@viniciusdacal](https://github.com/viniciusdacal)! - fix(db): automatic jsonb parsing on SQLite/D1 reads, explicit stringify on writes, validator wired, JSONB path filter keys type-gated to Postgres
  ...
  **Behaviour changes**
  ...
  - **Path-shaped filter keys (`where: { 'meta->field': ... }`) are now a compile error on SQLite.** `createDb({ dialect: 'sqlite', ... })` narrows the new `TDialect` generic from the literal; the keyed-never brand `JsonbPathFilterGuard` surfaces the recovery sentence verbatim in the TS diagnostic: _"JSONB path filters require dialect: 'postgres'. On SQLite, fetch with list() and filter in application code."_ Postgres is unchanged — path keys compile and run as before. The pre-existing `where.ts` runtime throw remains as a backstop for the widened-variable case (`const dialect: DialectName = ...`).
...
- [#2856](https://github.com/vertz-dev/vertz/pull/2856) [`b37301c`](https://github.com/vertz-dev/vertz/commit/b37301c4e18b628e1740e8bf96552348d3aad354) Thanks [@viniciusdacal](https://github.com/viniciusdacal)! - fix(db): emit indexes, foreign keys, and enum CHECK constraints when autoApply generates DDL
