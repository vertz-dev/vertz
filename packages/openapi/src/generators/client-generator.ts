...
export function generateClient(
  ...
  const defaultBaseURL = config.baseURL ? `'${config.baseURL}'` : "''";
  const lines: string[] = [];
  ...
  lines.push('  post<T>(path: string, body?: unknown): Promise<T>;');
  lines.push('  put<T>(path: string, body?: unknown): Promise<T>;');
  lines.push('  patch<T>(path: string, body?: unknown): Promise<T>;');
  lines.push('  delete<T>(path: string): Promise<T>;');
  lines.push('}');
  lines.push('');
  ...
  for (const r of resources) {
    lines.push(
      `import { create${r.name}Resource } from './resources/${r.identifier}';`,
    );
  }
  lines.push('');
  ...
  // POST, PUT, PATCH — same pattern
  for (const method of ['post', 'put', 'patch'] as const) {
    lines.push(
      `    async ${method}<T>(path: string, body?: unknown): Promise<T> {`,
    );
    lines.push(`      const res = await customFetch(\`\${baseURL}\${path}\`, {`);
  ...
  // DELETE
  lines.push('    async delete<T>(path: string): Promise<T> {');
  lines.push('      const res = await customFetch(`${baseURL}${path}`, {');
