...
<Tip>Use unique values (timestamps, counters) in test seeds to avoid collisions when tests run in parallel.</Tip>
...
<Warning>Never seed user accounts, API keys, or sensitive data in production. Use environment variables and admin tooling for those.</Warning>
