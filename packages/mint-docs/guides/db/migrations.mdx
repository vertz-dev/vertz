...
<Steps>
  <Step title="Define your schema">
    Create a `.schema.ts` file with your table definitions using the `d` builder. See the [schema
    guide](/guides/db/schema) for details.
  </Step>
  <Step title="Start the dev server">
    Run `vertz dev`. The dev server watches your schema files and applies migrations automatically
    when you save.
  </Step>
  <Step title="Generate migration files before deploying">
    When you're ready to deploy, run `vertz db migrate` to create a migration file, then `vertz db
    deploy` in production to apply it.
  </Step>
</Steps>
...
- **`vertz db pull`** — connects to an existing database and generates a TypeScript schema file. See the [introspection guide](/guides/db/introspection).
