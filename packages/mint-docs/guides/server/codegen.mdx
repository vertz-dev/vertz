...
**2. Via `vtz exec vertz codegen`** — run codegen manually:

```bash
vtz exec vertz codegen
```
...
When no codegen output exists (e.g., before running `vtz exec vertz codegen` for the first time), the `Entitlement` type falls back to `string`. This means:
...
Running `vtz exec vertz codegen` produces:
