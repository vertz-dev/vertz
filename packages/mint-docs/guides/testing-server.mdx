...
The typed test client gives you 100% type-safe access to your server's entities and services in tests — no path strings, no manual casting, no guessing. Tests use the [built-in test runner](/guides/testing-unit) (`vtz test`) and import test utilities from `@vertz/test`.
