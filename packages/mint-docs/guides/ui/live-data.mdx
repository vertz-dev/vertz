...
The descriptor carries its own `_key` (derived from method + path + args, identical to REST descriptors), so cache identity, dedup, and `invalidate()` work the same way they do for `query(api.tasks.list({ page: 1 }))`. No manual key, no wrapper thunk.
