...
**Primitive components** (`Dialog`, `Select`, etc.) are themed wrappers around `@vertz/ui-primitives` with compound sub-components (e.g., `Dialog.Title`, `Dialog.Footer`, `Select.Content`).
</Note>
...
<EmptyState>
  ...
  <EmptyState.Action>
    <Button intent="primary" size="sm">New Issue</Button>
  </EmptyState.Action>
</EmptyState>
```
  ...
  return (
    <>
      ...
      <Dialog.Footer>
        ...
        <Button intent="primary" size="md" onClick={() => dialog.close()}>Save changes</Button>
      </Dialog.Footer>
    </>
  );
}
...
function DeleteButton() {
  ...
  return <Button intent="destructive" size="md" onClick={handleDelete}>Delete Account</Button>;
}
```
