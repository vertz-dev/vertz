...
### AOT rendering in production

When you run `vertz build`, the build pipeline automatically generates `aot-manifest.json` and `aot-routes.js` in `dist/server/`. To use AOT rendering in production, load the manifest at startup and pass it to the handler:
...
// Load AOT manifest (returns null if not available)
...
When `aotManifest` is provided, routes that match an AOT entry are rendered via pre-compiled string-builder functions — bypassing the DOM shim entirely. Routes without AOT entries automatically fall back to `ssrRenderSinglePass()`. This is fully transparent: the HTML output is identical either way.

`loadAotManifest()` returns `null` if the manifest files don't exist, so you can always call it — it degrades gracefully when AOT compilation is not available.

<Note>
`vertz start` loads the AOT manifest automatically. You only need `loadAotManifest()` when writing a custom production server.
</Note>
