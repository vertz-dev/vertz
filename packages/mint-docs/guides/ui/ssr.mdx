...
<Note>
If you're writing a custom production server instead of using `vertz start`, call `loadAotManifest()` to load the manifest and pass it as the `aotManifest` option to `createSSRHandler()`. See the [custom server example](#custom-server-with-aot) below.
</Note>
