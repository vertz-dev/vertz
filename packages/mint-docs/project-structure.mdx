...
<Note>
  The [native runtime](/runtime) (`vtz dev`) is the primary dev server. The Bun-based fallback
  (`bunx vertz dev` without the runtime installed) supports additional legacy paths:

- **Alternative directory:** `src/api/server.ts`
- **Additional extension:** `.js`

Use `src/server.ts` or `src/server.tsx` for compatibility with both.
