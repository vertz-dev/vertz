...
<Warning>
  The default value for `compiler.entryFile` is `src/app.ts`. You only need to set it if your entity
  definitions live in a different file than the default. The codegen docs show `entryFile:
  'src/api/server.ts'` as an example — that configures where codegen looks for entities, not where
