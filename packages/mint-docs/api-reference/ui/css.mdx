...
**Property groups with validated values:** spacing (`p`, `m`, `gap`, etc.), color (`bg`), radius (`rounded`), shadow, alignment (`items`, `justify`), font weight (`weight`), line height (`leading`), content.
...
**Position offsets:** `top`, `right`, `bottom`, `left` (spacing scale or raw CSS), `inset` (all four).

**Object fit & aspect ratio:** `object` (cover, contain, fill, none, scale-down), `aspect` (auto, square, video, photo, or raw ratio).

**Raw properties (any value accepted):** `cursor`, `transition`, `resize`, `opacity`, `z`, `tracking`, `decoration`, `grid-cols`, `border-r/l/t/b`, `vt-name`, `view-transition-name`, `overflow`, `overflow-x`, `overflow-y`.
...
Output: `background-color: color-mix(in oklch, var(--color-primary) 50%, transparent)`
...
Works with all size properties (`w`, `h`, `min-w`, `max-w`, `min-h`, `max-h`). Fractions greater than 1 (e.g. `w:3/2` → `150%`) are allowed.
...
The `overflow-hidden` keyword also remains available for backward compatibility.
