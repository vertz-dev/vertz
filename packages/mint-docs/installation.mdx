...
<Note>
  The Vertz CLI automatically uses the [native runtime](/runtime) when available — a standalone
  binary that starts in ~5ms with built-in SSR, HMR, and test runner. No configuration needed.
  If the runtime isn't installed, the CLI falls back to Bun transparently.
</Note>
