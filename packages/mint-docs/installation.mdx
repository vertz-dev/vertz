...
<Info>
  **AI coding agents:** Use `vtz create vertz my-app` above instead. The manual setup below is
  for adding Vertz to an existing project that already has its own build pipeline.
</Info>
