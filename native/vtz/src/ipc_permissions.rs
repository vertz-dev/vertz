...
pub fn suggest_capability(method: &str) -> Option<&'static str> {
    match method {
        "fs.readTextFile" | "fs.readBinaryFile" | "fs.exists" | "fs.stat" | "fs.readDir" => {
            Some("fs:read")
        }
        "fs.writeTextFile" | "fs.writeBinaryFile" | "fs.createDir" | "fs.remove" | "fs.rename" => {
            Some("fs:write")
        }
...
mod tests {
    ...
    fn fs_read_allows_read_methods() {
        ...
        assert!(perms.is_allowed("fs.readTextFile"));
        assert!(perms.is_allowed("fs.readBinaryFile"));
    ...
    fn fs_read_denies_write_methods() {
        ...
        assert!(!perms.is_allowed("fs.writeBinaryFile"));
    ...
    fn suggest_capability_fs_read_methods() {
        ...
        assert_eq!(suggest_capability("fs.readBinaryFile"), Some("fs:read"));
    ...
    fn suggest_capability_fs_write_methods() {
        ...
        assert_eq!(suggest_capability("fs.writeBinaryFile"), Some("fs:write"));
