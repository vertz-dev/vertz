...
describe('Feature: getBinaryPath() resolves platform binary', () => {
  describe('Given a platform package is installed at the expected path', () => {
    describe('When getBinaryPath() is called', () => {
      it('Then returns the full path to the vertz-runtime binary', async () => {
        ...
        // getBinaryPath() should resolve to npm/runtime-<platform>-<arch>/vertz-runtime
        try {
          const result = getBinaryPath();
          // If resolution succeeds, the path should end with 'vertz-runtime'
...
describe('Feature: getBinaryPath() resolves correct path structure', () => {
  describe('Given the current platform is darwin-arm64', () => {
    describe('When getBinaryPath() resolves the package', () => {
      it('Then the returned path is <pkgDir>/vertz-runtime', async () => {
        // This test verifies the path construction: dirname(resolve(pkg/package.json)) + /vertz-runtime
        ...
        try {
          const result = getBinaryPath();
          // Path should be: <somewhere>/runtime-<platform>-<arch>/vertz-runtime
          const dir = dirname(result);
          const basename = result.split('/').pop();
          expect(basename).toBe('vertz-runtime');
        } catch {
