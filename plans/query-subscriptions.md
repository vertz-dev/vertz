...
- **Stream-typed `QueryDescriptor` — RESOLVED in #2855 / `plans/stream-descriptor.md`.** This non-goal was originally framed as deferred indefinitely, with codegen committing to function-thunk wrappers. After PR #2852 merged, articulating the actual call-site DX cost of manual cache keys (compared to REST's `query(api.tasks.list(...))` auto-derivation) inverted the call. A separate `StreamDescriptor<T>` type now lives in `@vertz/fetch` and `query()` recognizes it via a new overload — the SDK call site for stream endpoints reads identically to REST. Function-thunk overload kept as the escape hatch for ad-hoc / hand-rolled iterables.
