...
### FIXED: DTS generic type preservation

**Status: RESOLVED** (fixed in `fix/dts-type-preservation` branch)

When this demo was originally written, the `.d.ts` files emitted by `@vertz/db`
had all generic type parameters erased. This meant:

- `db.findOne('users', { where: { id } })` returned `unknown` instead of the
  properly typed user object
- `db.create('users', { data: {...} })` did not type-check the `data` fields
- `where` clauses accepted any `Record<string, unknown>` instead of validating
  field names and value types
- Service functions had to cast every result: `user as Record<string, unknown>`

The original code had `Record<string, unknown>` workarounds throughout every
service file. Now with the DTS fix:

- `db.findOne('users')` returns `{ id: string; email: string; name: string; role: "admin" | "member"; createdAt: Date } | null`
- `db.create('tasks', { data: {...} })` validates that all required fields are
  present and correctly typed (e.g., `status` must be `"todo" | "in_progress" | "done"`)
- `where` clauses are typed via `FilterType<T>` -- invalid field names produce
  compile errors, and values must match the column type
- `include: { assignee: true }` returns the related user with full type inference
- Service functions use `typeof users.$infer` and `typeof tasks.$update` directly
...
The type flow is now what you'd expect from a modern ORM. Zero casts needed in
the database layer. The `$infer`, `$insert`, and `$update` phantom types on table
definitions are a powerful pattern -- they give you the exact type for each
operation without any manual type definitions.

**New win: `FilterType` for dynamic where clauses.**
```typescript
import type { FilterType } from '@vertz/db';
type TaskFilter = FilterType<typeof tasks._columns>;

const where: TaskFilter = {};
if (input.status) where.status = input.status;  // autocomplete + type-checked
```

This replaced `const where: Record<string, unknown> = {}` which had no type safety.
...
6. ~~**DTS type erasure in `@vertz/db`**~~ -- **FIXED.** Previously all generic
   type parameters were erased in the published `.d.ts` files, forcing
   `Record<string, unknown>` workarounds throughout service code. The fix in
   `fix/dts-type-preservation` restores full type inference for `where`, `data`,
   `select`, `include`, and return types. This was originally the #1 issue in the
   database layer but is no longer a blocker.
...
9. **Full @vertz/db type inference (post-DTS fix)** -- `$infer`, `$insert`, and
   `$update` phantom types on table definitions give you the exact row type for
   each operation. `FilterType<T>` provides typed where clauses. Zero manual type
   definitions needed for database types -- they are all derived from the schema.
...
- **Post-DTS fix:** Removed all `Record<string, unknown>` casts from service files.
  `serializeUser` now takes `typeof users.$infer` directly. `serializeTask` takes
  `typeof tasks.$infer & { assignee?: User | null }` (the intersection handles the
  optional include). Where clauses use `FilterType<typeof tasks._columns>`. Update
  payloads use `typeof tasks.$update`. The code went from "works but lies about types"
  to "fully type-safe, no workarounds." This is the DX story we want to tell.
