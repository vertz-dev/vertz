...
const createTaskSchema = {
    parse(data) {
        ...
        if (!obj.priority || !validPriorities.includes(obj.priority)) {
...
export function TaskForm(props) {
    ...
    const submitBtn = (<button type="submit" class={button({ intent: 'primary', size: 'md' })} data-testid="submit-task">
      Create Task
    </button>);
    ...
    const formEl = (<form action={formAttrs.action} method={formAttrs.method} data-testid="create-task-form">
      <div class={formStyles.classNames.formGroup}>
        <label class={formStyles.classNames.label} for="task-title">
          Title
        </label>
        <input class={formStyles.classNames.input} id="task-title" name="title" type="text" placeholder="What needs to be done?"/>
        {titleError}
      </div>
      ...
      <div class={formStyles.classNames.formGroup}>
        ...
        <textarea class={formStyles.classNames.textarea} id="task-description" name="description" placeholder="Describe the task in detail..."/>
        {descError}
      </div>
    ...
    // Submit handler
    formEl.addEventListener('submit', taskForm.handleSubmit({
        onSuccess: (task) => {
            onSuccess(task);
        },
        onError: (errors) => {
