...
export function TaskDetailPage(props) {
    ...
    const descBody = (<div class={detailStyles.classNames.description} data-testid="task-description"/>);
    ...
    detailsTab.panel.appendChild((<div class={detailStyles.classNames.section}>
        <h3 class={detailStyles.classNames.sectionTitle}>Description</h3>
        {descBody}
      </div>));
    // Activity panel content
    activityTab.panel.appendChild((<div class={detailStyles.classNames.timeline}>
    ...
    effect(() => {
        ...
        statusBar.innerHTML = '';
        statusBar.appendChild((<span class={badge({
        ...
        for (const transition of transitions) {
            statusBar.appendChild((<button class={button({ intent: 'secondary', size: 'sm' })} onClick={async () => {
        ...
        actionsEl.innerHTML = '';
        actionsEl.appendChild((<ConfirmDialog triggerLabel="Delete" title="Delete Task" description={`Are you sure you want to delete "${t.title}"? This action cannot be undone.`} confirmLabel="Delete Task" onConfirm={async () => {
    ...
    return (<div class={detailStyles.classNames.page} data-testid="task-detail-page">
      {isLoading && <div data-testid="loading">Loading task...</div>}
      {hasError && (<div style="color: var(--color-danger-500)" data-testid="error">
          {errorMsg}
        </div>)}
      {!isLoading && !hasError && task && (<div data-testid="task-content">
          <button class={button({ intent: 'ghost', size: 'sm' })} style="margin-bottom: 1rem" onClick={() => navigate('/')}>
            Back to Tasks
          </button>
