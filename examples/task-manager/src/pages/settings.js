...
export function SettingsPage(_props) {
    ...
    const lightCard = (<div data-testid="theme-light" role="button" tabindex="0" onClick={() => {
    ...
    const darkCard = (<div data-testid="theme-dark" role="button" tabindex="0" onClick={() => {
    ...
    effect(() => {
        ...
        lightCard.className = `${settingsStyles.classNames.themeCard} ${current === 'light'
            ? settingsStyles.classNames.themeCardActive
            : settingsStyles.classNames.themeCardInactive}`;
        darkCard.className = `${settingsStyles.classNames.themeCard} ${current === 'dark'
            ? settingsStyles.classNames.themeCardActive
            : settingsStyles.classNames.themeCardInactive}`;
    });
    // ── Priority select ─────────────────────────────────
    const prioritySelect = (<select class={formStyles.classNames.select} data-testid="default-priority-select">
    ...
    watch(() => settings.theme.value, (newTheme) => {
        console.log(`Theme changed to: ${newTheme}`);
    });
    // ── Page layout with JSX ────────────────────────────
    return (<div class={settingsStyles.classNames.page} data-testid="settings-page">
      ...
      <section class={settingsStyles.classNames.section}>
        ...
        {showSaved && (<div class={settingsStyles.classNames.savedMsg} data-testid="saved-message">
            Settings saved!
          </div>)}
      </section>
